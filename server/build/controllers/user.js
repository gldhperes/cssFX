...
export const signin = async (req, res) => {
  try {
    ...
    if (!existingUser || !isPasswordCorrect) return res.status(404).json({
...
export const favoritePost = async (req, res) => {
  try {
    ...
    if (user.favoritedPosts.includes(postId)) {
      updatedFavPost = await User.findByIdAndUpdate(userId, {
        $pull: {
    ...
    const favPost = await PostMessage.find({
      _id: posts.toString().split(',')
    });
    res.status(200).json({
      favPost: favPost
    });
...
export const follow = async (req, res) => {
  try {
    ...
    if (user.following.includes(postCreator)) {
      updatedFollowing = await User.findByIdAndUpdate(userId, {
        $pull: {
          following: postCreator
        }
      }, {
        new: true
      });
